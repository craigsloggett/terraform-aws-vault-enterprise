...
wait_for_bootstrap_election() (
  ...
  while [ "${attempt}" -lt "${max_attempts}" ]; do
    ...
    bootstrap_id="$(fetch_parameter "${BOOTSTRAP_NODE_ID_SSM_PARAMETER_NAME}" 2>/dev/null)" || bootstrap_id=""
...
main() {
  cluster_state="$(fetch_parameter "${BOOTSTRAP_CLUSTER_STATE_SSM_PARAMETER_NAME}" 2>/dev/null)" || cluster_state=""
  ...
  if is_bootstrap_node "${cluster_instance_ids}"; then
    ...
    put_parameter "${BOOTSTRAP_NODE_ID_SSM_PARAMETER_NAME}" "${INSTANCE_ID}"
  else
    wait_for_bootstrap_election
  fi
}
