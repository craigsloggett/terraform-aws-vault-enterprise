...
| <a name="input_enable_vault_agent"></a> [enable\_vault\_agent](#input\_enable\_vault\_agent) | Enable the Vault Agent service. When true (default), the agent handles initial TLS certificate issuance and ongoing rotation. When false, the legacy manual cert flow is used instead (preserved for rollback, scheduled for removal in a future release). | `bool` | `true` | no |
