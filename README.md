...
| <a name="input_hcp_terraform"></a> [hcp\_terraform](#input\_hcp\_terraform) | HCP Terraform JWT auth configuration for Terraform-managed Vault administration. When organization\_name is empty, JWT auth and the admin policy are not configured. | <pre>object({<br/>    hostname              = optional(string, "app.terraform.io")<br/>    organization_name     = optional(string, "")<br/>    workspace_id          = optional(string, "")<br/>    oidc_discovery_ca_pem = optional(string, "")<br/>  })</pre> | `{}` | no |
